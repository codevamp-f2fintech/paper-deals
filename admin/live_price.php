...
if (isset($_POST['save_Plan'])) {
    ...
    $query = "insert into live_price(name,price,location) values('$name','$price','$location')";
    $query_run = mysqli_query($conn, $query);

    if ($query_run) {
        $_SESSION['success'] = "Live Price Added Successfully!";
        header("Location: live_price.php");
    } else {
        $_SESSION['error'] = "Live Price Not Added Due to Some Error!";
        header("Location: live_price.php");
    }
...
if (isset($_POST['delete_plan_btn'])) {
    ...
    $query = "delete from live_price where id='$delete_plan_id'";
    $query_run = mysqli_query($conn, $query);

    if ($query_run) {
        $_SESSION['success'] = "Live Price Deleted Successfully!";
        header("Location: live_price.php");
    } else {
        $_SESSION['error'] = "Live Price Not Deleted Due to Some Error!";
        header("Location: live_price.php");
    }
}
?>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php echo $prod_item['location']; ?>
                                                </td> 
                                                <td>
                                                    <a href="live_price_edit.php?prod_id=<?php echo $prod_item['id']; ?>" style="width:100px ; text-align:center;border:1px solid #1C6C09;padding:4px; height:20px;font-size:13px; color:#1C6C09;background-color:#D9FBD0; border-radius:6px;">Edit</a>
                                                </td>
                                                <td>
                                                    <form  method="post">
