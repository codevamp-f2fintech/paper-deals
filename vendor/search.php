    ...
    <script>
        ...
        <section class="srch_sectn">
            ...
            <div class="container">
              ...
              $sqlserch= mysqli_query($conn,"SELECT users.id, CASE WHEN users.user_type = 2 THEN 1 ELSE 0 END AS SellerCount, CASE WHEN users.user_type = 3 THEN 1 ELSE 0 END AS buyerCount FROM users LEFT JOIN organization AS og ON users.id = og.user_id LEFT JOIN product_new AS p ON p.seller_id = users.id WHERE ((og.city LIKE '%$search%') OR (og.organizations LIKE '%$search%') OR (p.category_id LIKE '%$search%') OR (p.product_name LIKE '%$search%') OR (og.state LIKE '%$search%') OR (og.production_capacity LIKE '%$search%') OR (og.materials_used LIKE '%$search%') OR (og.district LIKE '%$search%') OR (og.city LIKE '%$search%'))");
...
// exit;
    ...
    WHERE 
        ...
        OR (p.product_name LIKE '%$search%') OR (og.state LIKE '%$search%') OR (og.production_capacity LIKE '%$search%') OR (og.materials_used LIKE '%$search%') OR (og.district LIKE '%$search%') OR (og.city LIKE '%$search%')) AND (users.user_type=3) GROUP by id";
                    ...
                    <?php 
                       ...
                       if($_GET['category_id']=="" && $_GET['cate_id']=="" && $_GET['p_name']=="" && $_REQUEST['search']==""){
                                
                               $query= "SELECT * FROM users LEFT JOIN organization ON users.id = organization.user_id LEFT JOIN state_list on state_list.state_id=organization.state WHERE users.user_type = 3 ORDER BY organization.verified DESC, organization.vip DESC";
 }
                               else if($_GET['category_id']!=""){
                                  
                                   $query= "SELECT * FROM product_new JOIN users ON users.id = product_new.seller_id JOIN organization ON product_new.seller_id = organization.user_id WHERE product_new.category_id = '$category' AND users.active_status=1 and users.user_type=3 GROUP BY product_new.seller_id ORDER BY organization.verified DESC, organization.vip DESC";
  
                               } else if($_GET['cate_id']!="" && $_GET['p_name']!=""){
                                   
                                    $query= "SELECT * FROM product_new JOIN users ON users.id = product_new.seller_id JOIN organization ON product_new.seller_id = organization.user_id WHERE product_new.product_name = '$product_name' AND users.active_status=1 and users.user_type=3 GROUP BY product_new.seller_id ORDER BY organization.verified DESC, organization.vip DESC";
                                   
                               }else if($_GET['category_id']=="" && $_GET['cate_id']=="" && $_GET['p_name']=="" && $_REQUEST['search']!=""){
    ...
    WHERE 
        ...
        OR (og.organizations LIKE '%$search%')
        OR (p.category_id LIKE '%$search%')
        OR (p.product_name LIKE '%$search%') OR (og.state LIKE '%$search%') OR (og.production_capacity LIKE '%$search%') OR (og.materials_used LIKE '%$search%') OR (og.district LIKE '%$search%') OR (og.city LIKE '%$search%')) AND (users.user_type=3) GROUP by users.id"; 
...
$product_name=$_GET['p_name'];
  ...
  if (isset($_REQUEST['submit'])) {
    ...
    WHERE 
        ...
        OR (og.organizations LIKE '%$search%')
        OR (p.category_id LIKE '%$search%')
        OR (p.product_name LIKE '%$search%') OR (og.state LIKE '%$search%') OR (og.production_capacity LIKE '%$search%') OR (og.materials_used LIKE '%$search%') OR (og.district LIKE '%$search%') OR (og.city LIKE '%$search%')) AND (users.user_type=2) GROUP by id";
...
if($_GET['category_id']=="" && $_GET['cate_id']=="" && $_GET['p_name']=="" && $_REQUEST['search']==""){
                               ...
                               $query= "SELECT * FROM users LEFT JOIN organization ON users.id = organization.user_id LEFT JOIN state_list on state_list.state_id=organization.state WHERE users.user_type = 2 ORDER BY organization.verified DESC, organization.vip DESC";
 }
                               else if($_GET['tab']=="seller" && $_GET['category_id']!=""){
    ...
    WHERE 
        ...
        OR (og.organizations LIKE '%$search%')
        OR (p.category_id LIKE '%$search%')
        OR (p.product_name LIKE '%$search%') OR (og.state LIKE '%$search%') OR (og.production_capacity LIKE '%$search%') OR (og.materials_used LIKE '%$search%') OR (og.district LIKE '%$search%') OR (og.city LIKE '%$search%')) AND (users.user_type=2) GROUP by users.id"; 
