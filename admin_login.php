	...
	<main class="mt-[70px] md:mt-0">
		...
		<section class="bg-[url('assets/herobg.jpg')] bg-cover lg:bg-fixed bg-no-repeat bg-bottom relative w-full h-fit">
			<div class="flex flex-col gap-2 items-center justify-center w-full bg-[#090909c4] px-4 py-16 md:py-24">
				<h1 class="text-2xl md:text-4xl font-bold text-white"><?php if ($_GET['type']) { ?> Guest <?php } else { ?>Admin <?php } ?> Login</h1>
		...
		<section class="sign_upsctn">

			<div class="container  admin rounded-md mt-10 mb-10 ">
				<div class="sgnupbx ">
					<div class="sell sgndata    w-full">

						<div class="media  ">
							<img src="https://paperdeals-doc-bucket.s3.ap-south-1.amazonaws.com/uploads/Privacy+policy-rafiki+(1).svg" class="mx-auto my-[50px]">
						</div>
					</div>
					<!-- ================================== -->
					<div class="sell sgn_frm  ">
						<div class="head">
							<h1 id="sign_in-head">Welcome Back !</h1>
							<p id="sign_in-Para">Please signin to continue</p>
						</div>
						...
						<form class="" action="" method="POST">
							<div class="sgnbx">
								...
								$message = $emailErr = $passwordErr = $captchaErr = "";
								if (isset($_REQUEST['submit'])) {
									...
									if (empty($recaptchaResponse)) {
										$captchaErr = 'Please complete the reCAPTCHA.';
									} else {
										// reCAPTCHA secret key
										$secretKey = '6LdQYVYqAAAAAM_1MtPnkAcybtgZo5Qsx5Jj653g'; // Replace this with your reCAPTCHA secret key
										...
										if (intval($responseKeys["success"]) !== 1) {
											$captchaErr = 'reCAPTCHA verification failed. Please try again.';
										}
									}
									...
									if ($emailErr == "" && $passwordErr == "" && $captchaErr == "") {
										$result = mysqli_query($conn, "SELECT * FROM users WHERE email_address = '$email_id' AND password = '$password' AND user_type = '$role' AND active_status=1 AND log_counter<5");
										$count_row = mysqli_num_rows($result);
										if ($count_row > 0) {
											$row = mysqli_fetch_array($result);
											if (is_array($row)) {

												setcookie("name_user", "test", time() + (86400 * 30), "/", "", true, true);
								...
								?>
												<div id="alert-2" class="flex items-center p-4 mb-4 text-red-800 rounded-lg bg-red-100 text-red-400" role="alert">
								...
								<div class="relative form-group ">
									<select id="type" name="type" class="block w-full pl-[14px] mb-6 text-sm text-gray-900 border rounded   bg-transparent  focus:ring-blue-500 focus:border-blue-500 dark:bg-transparent  dark:placeholder-gray-400 h-[56px] outline-none dark:focus:ring-blue-500 dark:focus:border-blue-500 peer">
										<option value="1" style="font-size:18px; padding-bottom:5%">Admin</option>
										<option value="4" style="font-size:18px; padding-bottom:5%">Super Admin</option>
										<option value="5" style="font-size:18px; padding-bottom:5%">Consultant</option>
										<option value="6" style="font-size:18px; padding-bottom:5%" <?php if ($_GET['type']) { ?>Selected <?php } ?>>Guest</option>
									</select>
								...
								<div class="form-button">
									...
									<p><a href="forgotpassword.php" class="mb-4">Forgot Password ?</a></p>
								</div>
